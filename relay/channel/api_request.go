...
var passthroughSkipHeaderNamesLower = map[string]struct{}{
	...
	// 路由控制头：由我们计算并下发（见 applyGPUStackAffinityHeader），绝不能接受
	// 客户端传入。GPUStack 网关的 set-model-pre-route 插件把这个头当 Envoy cluster
	// 名用，能把请求定向到任意一个实例；而 applyGPUStackAffinityHeader 刻意「已有值
	// 就不覆盖」（让运营显式配的 override 优先）。两条合起来意味着：只要某个渠道开了
	// 通配透传，终端用户就能自己指定落到哪个实例——包括它无权使用的另一个模型的实例。
	// 挡在这里而不是依赖「运营别开透传」，是因为前者是代码保证、后者是配置约定。
