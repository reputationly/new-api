...
我们曾照着那套写过一份（`relay/hilo/*` 的 IR 层 + `mode=ir`），移植时间 09-14，
比上游废弃它晚了四天，**且从未在生产启用过**。已于 09-16 整体删除（约 4500 行），
singlecall 是唯一的编译路径。

存量配置里若还写着 `"mode": "ir"`，干跑校验会报「不认识」并指到这份文档 ——
改成 `"singlecall"` 即可。

教训：**照当前工作流文档移植，不要照着仓库里还能跑的函数移植。**
