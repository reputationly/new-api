...
func DryRunAggregateModel(m *common.AggregateModel, peers map[string]int) *AggregateDryRunResult {
	...
	if m.PromptEnhance.IsEnabled() {
		...
		// 那是安全的一侧,但配置的人要的是 singlecall —— 他看不到任何异常,
		// 只会觉得"增强好像没起作用"。这里必须把它说出来。
		//
		// 已删除的 "ir" 也会落进这一支:那正是我们想要的 —— 存量配置里
		...
		if m.PromptEnhance.EnhanceMode() != common.EnhanceModeQwenPE &&
			...
			if mode := m.PromptEnhance.EnhanceMode(); mode == common.EnhanceModeSingleCall {
