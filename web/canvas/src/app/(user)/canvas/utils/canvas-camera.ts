 ...
 *
 * 组装出的提示词按「职责分段」拼接:
 *   1) 任务上下文:声明以下是相机/镜头/曝光参数
 *   2) 机身特性:传感器、色彩科学、整体质感
 *   3) 镜头特性:光学个性、虚化形状、耀斑表现
 *   4) 焦距:透视关系(广角/标准/长焦)与构图含义
 *   5) 光圈:景深与虚化强度
 *   6) 护栏:不要把相机本身画进画面
 *
 * 结果会拼进用户的生成提示词,因此每个片段都短且可用逗号连接。
 */
...
/**
 * 把摄像机配置拼进生成提示词。未启用或未配置时原样返回,
 * 因此可以无条件在各生成路径上调用。
 */
export function appendCameraPrompt(prompt: string, control?: CameraControlOptions): string {
...
/** 面板与截图节点共用的一行中文摘要 */
export function describeCameraControl(control?: CameraControlOptions): string {
    if (!control?.enabled) return "";
    const camera = CAMERA_PROFILES.find((item) => item.id === control.camera) ?? CAMERA_PROFILES[0];
    const lens = LENS_PROFILES.find((item) => item.id === control.lens) ?? LENS_PROFILES[0];
    return `${camera.zhName} · ${lens.zhName} · ${control.focalLength}mm · f/${control.aperture}`;
}
