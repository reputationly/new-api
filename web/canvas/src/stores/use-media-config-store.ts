...
export const useMediaConfigStore = create<MediaConfigStore>()((set, get) => ({
    ...
    refresh: async () => {
        ...
        try {
            ...
            const [configs, availableModels, groups, modelGroups] = await Promise.all([fetchMediaConfigs(), fetchAvailableModels(), fetchUserGroups().catch(() => [] as GroupOption[]), fetchModelGroups().catch(() => ({}) as Record<string, string[]>)]);
