...
import { groupRectFor } from "@/lib/canvas/canvas-node-geometry";
...
export function arrangeNodesOps(input: Record<string, unknown>, snapshot: CanvasAgentSnapshot): CanvasAgentOp[] {
    ...
    for (const index of [...byLayer.keys()].sort((a, b) => a - b)) {
        ...
        for (const node of nodes) {
            const width = node.width || NODE_DEFAULT_SIZE[node.type as CanvasNodeType]?.width || 320;
            const height = node.height || NODE_DEFAULT_SIZE[node.type as CanvasNodeType]?.height || 220;
