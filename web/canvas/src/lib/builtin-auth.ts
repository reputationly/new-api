...
//
// 画布与 new-api SPA 同源部署,session cookie 由浏览器自动携带;
// new-api 的 UserAuth 中间件额外要求 `New-Api-User: <uid>` 头(缺失直接 401),
// uid 由 SPA 登录后写入 localStorage。

import axios from "axios";

// 直接读构建期常量,避免与 use-config-store 形成循环依赖
const BUILTIN = __BUILTIN_MODE__;

// 内置模式全局兜底:站内 API 的 401 统一跳登录重建会话。
// 只拦 /pg 与 /api/ ——外部地址的 401 不该把用户踢去登录页。
...
export function builtinHeaders(): Record<string, string> {
    if (!BUILTIN || typeof localStorage === "undefined") return {};
