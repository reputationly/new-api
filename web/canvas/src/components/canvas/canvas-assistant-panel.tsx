...
import { CanvasPromptLibrary } from "@/components/canvas/canvas-prompt-library";
import { AgentChatComposer, AgentChatMessage, AgentModeSwitch, AgentPanelTabs, AgentWorkingMessage, type CanvasAgentChatMessage, type CanvasAgentMode } from "@/components/canvas/canvas-agent-chat-ui";
import { LocalAgentPanel as CanvasLocalAgentPanel } from "@/components/agent/local-agent-panel";
import { buildCanvasAgentSkillPrompt } from "@/lib/canvas/agent/canvas-agent-skills";
import { CAPABILITY_AGENT_TOOLS, capabilityNodeOps, concatVideosOps, extractVideoFrameOps, listCapabilitiesResult } from "@/lib/canvas/agent/capability-tools";
import { GRAPH_AGENT_TOOLS, arrangeNodesOps, collectWaitTargets, createGroupOps, getNodeResult, summarizeWaitResult, traverseResult } from "@/lib/canvas/agent/graph-tools";
    ...
    return (
        ...
        >
            ...
            >
                ...
                {/* 上游的本地 Agent 面板已改为自己从 store 取画布快照与 ops 通道,不再从 props 收 */}
                {agentMode === "local" ? <CanvasLocalAgentPanel embedded autoConnect={autoConnectLocal} /> : onlineContent}
            </motion.aside>
        </motion.div>
    );
}
