...
export const PLAYGROUND_FIELD_META = {
  ...
  aspectRatios: {
    ...
    help: '留空=该 tab 不展示宽高比选择器。图像玩法里它与下面的「画质档」**必须成对配置**：两者都配=按「画质档 × 比例」算出精确像素下发（推荐）；只配比例、不勾画质档则整份都不生效，画幅仍由上面的「尺寸 / 分辨率」列表决定（想直接下发比例词请把它填进那份列表）。',
  },
  ...
  sizeTiers: {
    label: '画质档',
    type: 'tiers',
    help: '勾选这个模型对外提供哪几档画质，与上面的「宽高比」组合算出精确像素下发。**两者必须成对配置**：三档都不勾的话，宽高比选择器也会一起消失（只配一半不构成 area 模式），画幅回落到上面的「尺寸 / 分辨率」列表。⚠️ **勾之前务必实测一次**：引擎超不了限时不会报错，而是按比例缩回自己的上限带，界面写的尺寸和实际出图就对不上了（这种情况请在下方「长边上限」里填上引擎的护栏值，界面会跟着钳制，就不会说谎）。第三方模型（gpt-image / dall-e / nano-banana 等）只认各自枚举的固定尺寸，**不要给它们勾画质档**，用上面的「尺寸/分辨率」列表。',
  },
  maxInputMB: {
    label: '上传文件上限（MB）',
    type: 'int',
...
export const PLAYGROUND_MODEL_LEVEL_FIELDS = {
  ImageModelSizeConfig: [
    ...
    {
      // 引擎侧长边护栏的镜像。与「画质档」不同,这里照常填数字 —— 它不是产品决策,
      // 而是引擎配置的复刻(LightX2V 四个图像 runner 的 max_custom_size,默认 1664),
      // 运营要做的是把引擎那边的值抄过来,不是自己定一个。
      key: 'maxLongEdge',
      label: '长边上限（像素）',
      type: 'int',
      // **不能写「0 = 不限」**:模型级 int 走的是 TabPanel 里内联的那套渲染器
      // (min={1}),不是 FieldInput(min={0})。Semi 的 InputNumber 会在 blur 时把
      // 越界值夹回 min,于是运营照着提示敲 0、实际存进去的是 1 —— 而 1 会被
      // computeImageSize 当成有效护栏,长边缩到 1、再被 floorTo 抬回 align,
      // 该模型每个比例每个档位都出 32x32,全程不报错。清空的唯一正确姿势是留空。
      // 姊妹字段 sizeAlign 的 placeholder 只写「留空=32」,同一个理由。
      placeholder: '留空=不限',
      help: '画质档算出的尺寸若长边超过它,界面会按长边等比缩回 —— 与引擎内部的做法逐字一致,保证显示的尺寸就是实际出图的尺寸。不填的话引擎照样会缩,只是界面不知道,于是写着 2048×1152、实际出 1664×928。实测 z-image / qwen-image / qwen-image-edit 的护栏是 1664,U1.5 / Ideogram-4 / Krea2 不设此限(它们受限于显存面积而不是长边)。属模型能力,与 tab 无关。',
    },
    {
      key: 'engine',
      label: '引擎族',
