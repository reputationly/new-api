...
export const PLAYGROUND_MODEL_LEVEL_FIELDS = {
  VideoModelConfig: [
    ...
    },
    {
      ...
      label: '超分档位',
      type: 'upscale',
      help: '给这个模型额外提供「先低档位生成、再交超分模型放大」的尺寸档位。留空=不提供（纯 opt-in）。起步档位必选，候选取该模型已配的、比目标更低的档位——没选或目标档位本身已在已配档位里，这条规则不生效。目标档位有两种写法，行为不同：①**精确像素串**（如 2560x1440、3840x2160）——取其**短边**下发给引擎，引擎按源视频的真实画幅等比放大到该短边：画幅零形变、短边精确命中，一个超分部署就能同时提供 1080P/2K/4K（SwiftVR 支持）。注意出片的长边由源决定而非照抄这里写的数，例如源是 1344×768（H3 的 768P 实际画幅）写 2560x1440 会出2520×1440——这是对的，硬凑 2560 反而要拉伸 1.6%。②**档位词**（如 1080P）——不下发尺寸，退回「超分模型按它自己部署配置里的分辨率档位出片」的老行为（现网 seedvr2 各档均为 1920×1080），此时目标档位只是界面标签，要照着超分模型的实际部署填，填了别的不会改变出片分辨率、只会让界面标错。⚠️ 别写 2K / 4K 这类词：尺寸解析器不认（它与后端 H3 的档位解析是刻意同口径的一对，不能为超分单方面加词），会当成档位词走 ② 的老路。',
    },
    {
      key: 'engine',
      label: '引擎族',
